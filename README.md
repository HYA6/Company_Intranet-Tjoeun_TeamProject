# Company Intranet Project 🏙

## 📔 목차
* 📃 [프로젝트 개요](#-프로젝트-개요)
* 🛠 [사용한 기술](#-사용한-기술)
* 👨‍💻 [팀 내 역할](#-팀-내-역할)
* 💡 [주요 기능](#-주요-기능)
* 🔍 [맡은 기능](#-맡은-기능)
  * [전자 결재 경영진 페이지](#전자-결재-경영진-페이지)
  * [사내 일정](#사내-일정)
  * [메인 페이지](#메인-페이지)
  * [회의실 예약](#회의실-예약)
  * [공지사항](#공지사항)
  * [기타](#기타)
* ✍ [프로젝트 후 느낀 점](#-프로젝트-후-느낀-점)
* ✔ [추후 유지 보수 및 기능 추가](#-추후-유지-보수-및-기능-추가)
  * [유지 보수](#유지-보수)
  * [배포](#배포)
  * [기능 추가](#기능-추가)

<br/>

## 📃 프로젝트 개요
<img src="https://github.com/HYA6/Company_Intranet/assets/130038444/890c55cb-1b8b-43b6-bae8-dfe06749b2ca" width="400"/>

> <b>주제</b> : 사내 인트라넷
> 
> <b>프로젝트 종류</b> : 팀 프로젝트
> 
> <b>수행인원</b> : 4명
> 
> <b>기간</b> : 2023.06.28 - 2023.09.19 (이후 개인적으로 유지 보수 중)
>
> <b>문의</b> : sjo6382@naver.com

<br/>

## 🛠 사용한 기술
  * <b>BACK-END</b><br/>
      * Java, Spring Framework, MyBatis, Ajax, Maven
  * <b>FRONT-END</b><br/>
      * JavaScript, JQuery, Jsp, JSTL, CSS, Bootstrap
  * <b>DB</b><br/>
      * Oracle Database
  * <b>SERVER</b><br/>
      * Tomcat
  * <b>TOOLS & COMMUNICATION</b><br/>
    * STS Eclipse, Notion, Kakaotalk

<br/>

## 👨‍💻 팀 내 역할
- <b>백엔드 개발 및 총괄 디렉터</b>
  - 백엔드
  - 프론트엔드
  - 일정 확인
  - 오류 및 코드 정리
  - 팀원들이 구현한 기능들 취합
  - 기능 테스트 후 기능 정리 및 추가해야 할 사항 정리

<br/>

## 💡 주요 기능
  * 전자 결재 / 사장 - 결재 승인, 반려<br/>
  * 회의실 예약<br/>
  * 출퇴근 기록<br/>
  * 마이페이지(직급, 연차, 연락처 등)<br/>
  * 사내 일정, 공지사항, 자유게시판, 식단표<br/>

<br/>

## 🔍 맡은 기능

- ### 맡은 주요 기능
  * 전자 결재 경영진 페이지 전체
  * 사내 일정 페이지 전체
  * 프론트엔드

- ### 전자 결재 경영진 페이지
  * 전자 결재 반려 사유 입력<br/>
    <img src="https://github.com/HYA6/Company_Intranet/assets/130038444/3533ae59-1dd6-481f-b445-20420cf49ebc"/><br/>
  * 전자 결재 반려 사유 수정<br/>
    <img src="https://github.com/HYA6/Company_Intranet/assets/130038444/868f0c0d-ed16-44fe-aca5-099037bea49c"/><br/>
  * 전자 결재 승인<br/>
    <img src="https://github.com/HYA6/Company_Intranet/assets/130038444/74a61cd7-a031-45f1-94a9-1e3b8a96180e"/><br/>
  * 전자 결재 종류별 페이지<br/>
    <img src="https://github.com/HYA6/Company_Intranet/assets/130038444/35fe889e-1a25-4438-a7f7-581f07d4e656"/><br/>
  * 전자 결재 검색 (결재 종류별, 제목, 내용, 이름)<br/>
    <img src="https://github.com/HYA6/Company_Intranet/assets/130038444/9cefd3cc-58f2-4467-b869-57c7631c5171"/><br/>
  * 전자 결재 작성, 수정, 삭제
  * 파일 첨부

- ### 사내 일정
  * 사내 일정 저장<br/>
    <img src="https://github.com/HYA6/Company_Intranet/assets/130038444/738d7100-0d0c-4dc7-80d7-61cc0ecc87b4"/><br/>
  * 사내 일정 수정<br/>
    <img src="https://github.com/HYA6/Company_Intranet/assets/130038444/054d1c1e-6b0e-450e-9723-eecba4da6c64"/><br/>
  * 사내 일정 삭제<br/>
    <img src="https://github.com/HYA6/Company_Intranet/assets/130038444/ef94938e-773c-4db8-8485-e7fbdade0eb5"/><br/>
  * 사내 일정 달력 검색 및 날짜 넘기기<br/>
    <img src="https://github.com/HYA6/Company_Intranet/assets/130038444/ae5d3ff8-4508-49c3-852a-95197936503b"/><br/>
  * 공휴일 및 대체 공휴일 설정<br/>

- ### 메인 페이지
  - 행사 사진 넘기기
  - 공지사항 최신글 5개 출력하기
  - 당일의 사내 일정 출력하기<br/>
  <img src="https://github.com/HYA6/Company_Intranet/assets/130038444/2baf9cd4-a4e2-46aa-9b5e-7d9221242c96"/><br/>

- ### 회의실 예약
  - 모달창에 예약 정보 띄우기
  - 예약되어 있는 시간 비활성화<br/>
    <img src="https://github.com/HYA6/Company_Intranet/assets/130038444/57adea2c-5016-4e7d-a776-2aeae3cf9add"/><br/>

- ### 공지사항
  - 주요 공지 글목록에서 맨 위로 띄우기
  - 게시글 목록 띄우기, 게시글 작성 및 수정, 삭제, 페이지 넘김, 작성 시 공지 종류 선택하기 등 전체적인 기능 구현 협력<br/>
    <img src="https://github.com/HYA6/Company_Intranet/assets/130038444/250214f3-2f9e-486a-830a-f67c990f0072"/><br/>

- ### 기타
  - 전자 결재 사원 페이지 : 결재 종류 선택 및 종류별 페이지 나눔, 검색 기능
  - 자유게시판 : 게시글 목록 띄우기, 게시글 작성 및 수정, 삭제, 페이지 넘김 등 전체적인 기능 구현 협력
  - 사이드 바 : 각 페이지 연결
  - CSS : 전체, 템플릿 사용 - 출처: https://html5up.net/

- ### 전체적인 오류 수정
  - 동작하지 않는 페이지 이동 버튼 수정
  - 작성한 내용 줄바꿈 적용
  - 입력한 내용이 없으면 경고창 띄우기
  - 기능 실현 후 돌아갈 페이지 지정

<br/>

## ✍ 프로젝트 후 느낀 점

* ### 좋았던 점
  * 전체적인 프로젝트의 흐름을 파악할 수 있었고 계획대로 진행되어 목표했던 기능들 전부 구현할 수 있었습니다.
  * 기획부터 개발까지 팀원들과 함께 회의하며 진행하면서 커뮤니케이션이 중요하다는 것을 느꼈습니다.
  * 웹 서비스의 실행되는 과정을 자세히 알 수 있었습니다.
  * 이론으로만 알고 있었던 Spring MVC 흐름을 보다 더 정확하게 이해할 수 있었습니다.
  * 개발할 땐 미처 생각하지 못했던 오류들을 기능 테스트를 통해 발견할 수 있었고 그로 인해 기능 테스트가 중요하다는 것을 알게 되었습니다.
* ### 아쉬웠던 점
  * 배포하지 못한 채로 팀 프로젝트가 끝나서 아쉬웠습니다.
  * 데이터 처리 속도나 비즈니스 로직을 제대로 고려하지 않고 개발했던 점이 아쉬웠습니다.
  * 당시에는 Git을 제대로 활용하지 못한 점이 아쉬웠습니다.

<br/>

## ✔ 추후 유지 보수 및 기능 추가

* ### 유지 보수
  * 발견된 문제점
    * 1회성 조건문이나 필요없는 예외 처리 등 지저분한 코드
    * 정의되지 않은 DB 연관 관계
    * 사진 첨부 없이 식단표 작성 시 500 오류 발생
  * 해결 방안
    * 전체적인 코드 분석 후 정리
    * DB 외래키에 따른 CASCADE 및 제약 조건 설정

* ### 배포
  * 배포 관련하여 공부 후 적용 예정

* ### 기능 추가
  * 쪽지 송수신 기능
  * 메일 송수신 기능
  * 경영진 및 관리자가 사용할 수 있는 사원 관리 시스템
    * 구성원 추가 및 삭제
    * 조직도 작성
    * 연차 설정
  * 연봉 테이블 계산
